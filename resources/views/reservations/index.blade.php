                            ...
                            <tbody class="border-collapse">
                                @foreach ($reservations as $reservation)
                                    <tr>
                                        ...
                                        <td class="px-2 border-2">{{ date('j / m / Y ', strtotime($reservation->date_start)) }}</td>
                                        <td class="px-2 border-2">{{ date('j / m / Y ', strtotime($reservation->date_end)) }}</td>
                                        <td class="px-2 border-2 flex flex-col gap-y-1">
                                            ...
                                            <form class="w-[48%]" action="{{ route('reservations.destroy', $hotel->id) }}" method="POST">
                                                ...
                                                <button class="bg-red-500 text-white p-1 rounded" type="submit">Supprimer</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
